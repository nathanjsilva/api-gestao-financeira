...
import CardCategoryForm from '../../components/card-categories/CardCategoryForm.vue'
...
async function handleSubmit() {
  ...
  await withLoading(async () => {
    try {
      await cardCategoryService.create(form)
      resetForm()
...
function startEdit(category) {
  ...
  editForm.active = Boolean(category.active)
  clearErrors()
}
...
async function handleEditSubmit() {
  ...
  await withLoading(async () => {
    try {
      await cardCategoryService.update(editingId.value, editForm)
      closeEditModal()
      await loadCategories()
    } catch (error) {
      setErrorsFromApi(error)
...
async function removeCategory(category) {
  ...
  await withLoading(async () => {
    try {
      await cardCategoryService.remove(category.id)
      await loadCategories()
    } catch (error) {
      setErrorsFromApi(error)
    }
...
<template>
  <section class="dashboard-shell mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <PageHeader
      eyebrow="Cartões"
      title="Categorias de cartão"
      description="Organize os gastos de cartão por categoria. Elas alimentam os filtros e gráficos das análises."
    />
              ...
              <tr v-for="category in paginatedItems" :key="category.id" class="border-b border-white/5">
                ...
                <td class="py-4">
                  <span
                    ...
                    :class="category.active ? 'bg-emerald-400/10 text-emerald-300' : 'bg-slate-500/10 text-slate-400'"
                  >
        ...
        <div v-if="categories.length" class="grid gap-3 md:hidden">
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <strong class="break-words text-lg text-slate-50">{{ category.name }}</strong>
              <span
                class="rounded-full px-3 py-1 text-xs font-bold"
                :class="category.active ? 'bg-emerald-400/10 text-emerald-300' : 'bg-slate-500/10 text-slate-400'"
              >
